...
<body>
    ...
    <!--视频右侧导航end-->
<link rel="stylesheet" href="../Public/Css/Content/specialinfo.css">
<div class="container">

    <section class="special_heading">
        <figure class="info">
            <img src="<?php echo ($spec['img']); ?>">
            <figcaption>
                <h1><?php echo ($spec['name']); ?></h1>
                <p class="special_descrip"><?php echo ($spec['describe']); ?></p>
            </figcaption>
        </figure>
    </section>
    <section class="special_box">
        <article>
            <section>
                <?php if(!empty($artis)): if(is_array($artis)): foreach($artis as $key=>$row2): ?><figure class="special_item">
                            <h3><a href="__URL__/artcon/id/<?php echo ($row2['id']); ?>"><?php echo ($row2['title']); ?></a> </h3>
                            <p>
                                <time><?php echo date('Y-m-d H:i',$row2['inputtime']);?></time>
                                <span>阅读数:<?php echo ($row2['views']); ?></span>
                            </p>
                            <figcaption>
                                <a href="__URL__/artcon/id/<?php echo ($row2['id']); ?>">
                                    <?php echo ($row2['img']); ?>
                                    <div class="arti_con">
                                        <?php echo mb_substr($row2['content'],0,200,'utf-8');?>
                                    </div>
                                </a>
                                <div class="clear"></div>
                            </figcaption>
                    ...
                    <?php else: ?>
                    <p class="special_empty">该专题下暂无文章</p><?php endif; ?>
                <?php echo ($show); ?>
            </section>
        </article>
        <aside>
            <section class="alsolike">
                <h3>更多专题</h3>
                <?php if(is_array($mores)): foreach($mores as $key=>$row3): ?><figure>
