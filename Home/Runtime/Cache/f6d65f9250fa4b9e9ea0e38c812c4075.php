...
<body>
    ...
    <!--视频右侧导航end-->
<link rel="stylesheet" href="../Public/Css/Content/artcon.css">
<script src="../Public/Js/Content/jquery.dotdotdot.min.js"></script>
<script>
    $(document).ready(function() {
        $(".about_item figcaption p a").dotdotdot({
...
<div class="container">
    <article>
        <section>
            <!--面包屑导航start-->
            <div class="crumbs">
                <?php if(is_array($nav)): foreach($nav as $key=>$row): if($key == 0): ?><a href="__URL__/article/id/<?php echo ($row['id']); ?>"><?php echo ($row['name']); ?></a><?php endif; endforeach; endif; ?>
                <span> > </span>
                <a href="__URL__/article/id/<?php echo ($nav[0]['id']); ?>/category/<?php echo ($cate['id']); ?>"><?php echo ($cate['name']); ?></a>
                <span> > </span>
                <span>正文</span>
           </div>
            <!--面包屑导航end-->
            ...
            <div class="message">
                <span>来源：</span>
                <span class="source"><?php echo htmlspecialchars_decode($art['source']);?></span>
                <span>发布时间：</span>
                <time><?php echo date('Y-m-d H:i',$art['inputtime']);?></time>
                <span>阅读数:</span>
                <span><?php echo ($art['weekviews']); ?></span>
                <span class="keywords"><span>关键词：</span><?php echo mb_substr($art['keywords'],0,10,'utf8');?></span>
            </div>
            <!--内容主体start-->
            <div class="content">
            ...
            <div class="sharandcoll">
                ...
                <!--分享，收藏end-->
                <!--点赞收藏start-->
                <iframe id='fabcoll' src="__APP__/Fabcoll/index/id/<?php echo ($_GET['id']); ?>/type/1"  scrolling="no" style="float: right">
                </iframe>
                <!--点赞收藏end-->
                <div class="clear"></div>
            </div>
        ...
        <!--加载评论模块start-->
        <iframe class='iframeId' src="__APP__/Comment/index/id/<?php echo ($_GET['id']); ?>/type/1" width="100%" scrolling="no"></iframe>
        <!--加载评论模块end-->
    </article>
    ...
    <aside>
        ...
        <section class="about">
            ...
            <div class="about_box">
                <?php if(is_array($recommends)): foreach($recommends as $key=>$row): ?><figure class="about_item">
